...
export const DashboardHeader: React.FC<{ setups?: EdgeSetup[] }> = ({ setups = [] }) => {
  ...
  const [showMannaDropdown, setShowMannaDropdown] = useState(false);

  const handleTriggerMannaScan = async (market: 'both' | 'forex' | 'futures' = 'both') => {
    setIsScanningManna(true);
    setShowMannaDropdown(false);
    try {
      const res = await fetch(`${API_BASE}/api/admin/manna-snd/scan`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ market, mode: 'live', force: false })
      });
            ...
            {isSuperAdmin && (
              <div style={{ position: 'relative' }}>
                <button
                  ...
                  onClick={() => setShowMannaDropdown(prev => !prev)}
                  disabled={isScanningManna}
                  title="Trigger manual Manna SnD scan (All Assets, Forex Only, or Futures Only)"
                >
                ...
                {showMannaDropdown && (
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={() => handleTriggerMannaScan('both')}
                    >
                    ...
                    <button
                      ...
                      onClick={() => handleTriggerMannaScan('forex')}
                    >
                    ...
                    <button
                      ...
                      onClick={() => handleTriggerMannaScan('futures')}
                    >
              ...
              {isSuperAdmin && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '8px' }}>
                  <button 
                    ...
                    onClick={() => { handleTriggerMannaScan('both'); setMobileMenuOpen(false); }}
                    disabled={isScanningManna}
                  >
                    <span>🟡 Scan All Assets (Manna SnD)</span>
                    <span style={{ fontSize: '0.68rem', opacity: 0.8 }}>Forex+CME</span>
                  </button>
                  <button 
                    className="mobile-nav-item font-mono"
                    style={{
                    ...
                    onClick={() => { handleTriggerMannaScan('forex'); setMobileMenuOpen(false); }}
                    disabled={isScanningManna}
                  >
                    <span>💱 Scan Forex Only</span>
                    <span style={{ fontSize: '0.68rem', opacity: 0.8 }}>FX Pairs</span>
                  </button>
                  <button 
                    className="mobile-nav-item font-mono"
                    style={{
                    ...
                    onClick={() => { handleTriggerMannaScan('futures'); setMobileMenuOpen(false); }}
                    disabled={isScanningManna}
                  >
                    <span>📊 Scan Futures Only</span>
                    <span style={{ fontSize: '0.68rem', opacity: 0.8 }}>CME</span>
                  </button>
                </div>
              )}
