...
function playChimeOnly(onEnded?: () => void) {
  try {
...
export const VoiceProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Global Audio Unlocker on User Click
  useEffect(() => {
    const unlockAudio = () => {
      if (isUnlockedRef.current) return;
      isUnlockedRef.current = true;
      console.log('[Manna Voice] 🔓 Unlocking browser audio engine');
  ...
  /**
   * Process speech queue using audio.onended completion sequence
   */
  const processQueue = useCallback(() => {
    ...
    // 1. Play alert chime first, and speak AFTER the chime finishes
    playChimeOnly(() => {
      try {
