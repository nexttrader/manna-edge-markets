...
assert.strictEqual(res1B.reason, 'target_reached_pre_entry');
...
assert.strictEqual(res1C.reason, 'target_reached_pre_entry');
...
assert.strictEqual(res1D.reason, 'target_reached_pre_entry');
...
assert.strictEqual(res1E.reason, 'stop_breached_pre_entry');
assert.ok(res1E.detail?.includes('Stop Loss'), 'Detail must indicate Stop Loss was breached');

// Case 1F: Price blows through zone bottom by > 1.5x ATR (with wider SL)
const wideStopLong = { ...mockLongSetup, stop: 151.00 };
const res1F = revalidateSetup(wideStopLong, 152.60, 0.20, 153.50, 152.60); // 152.60 is 0.40 below 153.00 (> 1.5 * 0.20 = 0.30) but above 151.00 stop
assert.strictEqual(res1F.isValid, false, 'Price blown through zone bottom > 1.5x ATR must invalidate setup');
assert.strictEqual(res1F.reason, 'price_displaced');

console.log('✅ TEST 1: Long Pre-Entry Invalidation (TP1, TP2, Wick High, SL Breach, Zone Blow-Through) Passed');
...
assert.strictEqual(res2B.reason, 'target_reached_pre_entry');
...
assert.strictEqual(res2C.reason, 'target_reached_pre_entry');
...
assert.strictEqual(res2D.reason, 'target_reached_pre_entry');
...
assert.strictEqual(res2E.reason, 'stop_breached_pre_entry');

// Case 2F: Price blows through zone top by > 1.5x ATR (with wider SL)
const wideStopShort = { ...mockShortSetup, stop: 157.00 };
const res2F = revalidateSetup(wideStopShort, 155.60, 0.20, 155.60, 154.50); // 155.60 is 0.40 above 155.20 (> 1.5 * 0.20 = 0.30) but below 157.00 stop
assert.strictEqual(res2F.isValid, false, 'Short price blown through zone top > 1.5x ATR must invalidate setup');
assert.strictEqual(res2F.reason, 'price_displaced');

console.log('✅ TEST 2: Short Pre-Entry Invalidation (TP1, TP2, Wick Low, SL Breach, Zone Blow-Through) Passed');
