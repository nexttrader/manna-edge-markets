...
export async function executePublishRun(
  ...
  try {
        ...
        for (const key of Array.from(keysSet)) {
          ...
          if (existingSetup) {
            ...
            if (!revalResult.isValid) {
              queries.updateSetupState(existingSetup.id, market.name, 'invalidated', {
              ...
              hawkeyeService.logInvalidation({
          ...
          for (const inv of dedupeResult.invalidations) {
            if (inv.reason === 'discarded_duplicate') {
               ...
               queries.updateSetupState(effectiveExisting.id, market.name, 'superseded', {
                 superseded: 1,
                 tradable: 0,
    ...
    queries.updatePublishRun(runId, {
      run_state: 'committed',
    ...
    if (err.message === 'DRY_RUN_ROLLBACK') {
      ...
      logger.error({ err, runId }, 'Publish run failed — rolled back');
      queries.updatePublishRun(runId, {
        run_state: 'rolled_back',
        error_detail: err.message
      });
      metrics.increment('runs_total');
      metrics.increment('runs_rolled_back');
      
      return { 
        success: false, 
        runId, 
        mode: actualMode, 
        stats: { created: 0, invalidated: 0, preserved: 0, discarded: 0 }, 
        errors: [err.message] 
      };
    }
  }
