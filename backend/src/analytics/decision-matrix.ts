export interface DecisionMatrixFactorScores {
  ...
  killzone: number;     // 0 - 100
  newsSafety: number;   // 0 - 100
...
export function calculateAssetMatrixItem(
  ...
  if (price !== undefined && price > 0) {
    ...
    if (price >= minEntry && price <= maxEntry) {
      ...
      // Proximity drops off as distance in R increases
  ...
  // 3. Killzone Score (Weight: 15%)
  const kz = getCurrentKillzone(customTimestamp);
  const S_killzone = kz.score;

  // 4. News Safety Score (Weight: 10%)
  let S_news = 100;
  ...
  for (const event of newsEvents) {
    if (event.impact === 'high') {
      ...
      if (diffMinutes <= 15) {
        S_news = 15; // High risk imminent news trap
        break;
  ...
  // 5. Risk Reward Score (Weight: 10%)
  const S_rr = Math.min(100, Math.max(0, (rMultiple1 / 3.0) * 100));
  ...
  let statusLabel = 'Distant / Off-Killzone';

  if (priorityScore >= 90 || (isInZone && priorityScore >= 80)) {
    priorityTier = 'IMMINENT_FOCUS';
    actionableRec = 'EXECUTE_OR_ARM';
    statusLabel = isInZone ? 'IN EXECUTION ZONE' : 'IMMINENT ENTRY';
  } else if (priorityScore >= 75) {
    priorityTier = 'HIGH_ATTENTION';
    actionableRec = 'ARM_ORDER';
    statusLabel = 'APPROACHING ZONE';
  ...
  return {
    ...
    factors: {
      ...
      killzone: Math.round(S_killzone),
      newsSafety: Math.round(S_news),
